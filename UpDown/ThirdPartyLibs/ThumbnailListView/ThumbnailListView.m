...
- (void)reloadData
{
	NSInteger	itemCount = [_dataSource numberOfItemsInThumbnailListView:self];
...
- (void)reloadThumbnailImage
{
	...
	for( NSInteger i=0; i<itemCount; i++ )
    {
		...
		//thumbnailView.imageView.image = [_dataSource thumbnailListView:self imageAtIndex:i];
        
        NSString *imageUrlStr =  [_dataSource thumbnailListView:self imageAtIndex:i];
        
        [thumbnailView.imageView sd_setImageWithURL:[NSURL URLWithString: imageUrlStr]
